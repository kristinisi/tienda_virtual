...
document.addEventListener("DOMContentLoaded", function () {
  tableRoles = $("#tableRoles").dataTable({
    ...
    responsieve: "true",
    bDestroy: true,
    iDisplayLenght: 10, //mostrar los primeros 10 registros
    order: [[0, "desc"]],
  });
...
function fntEditRol() {
  ...
  btnEditRol.forEach(function (btnEdit) {
    btnEdit.addEventListener("click", function () {
      ...
      let idrol = this.getAttribute("rl"); //rl hace referencia al id del rol
      let request = window.XMLHttpRequest
        ? new XMLHttpRequest()
...
function fntDelRol() {
  ...
  btnDelRol.forEach(function (btnDelRol) {
    btnDelRol.addEventListener("click", function () {
      let idrol = this.getAttribute("rl"); //obtenemos el valor del atributo que es el id del rol
...
function fntPermisos() {
  ...
  btnPermisosRol.forEach(function (btnPermisosRol) {
    btnPermisosRol.addEventListener("click", function () {
      ...
      let idrol = this.getAttribute("rl"); //obtenemos el id de la fila al que vamos a darle los permisos
      let request = window.XMLHttpRequest
        ? new XMLHttpRequest()
        : new ActiveXObject("Microsoft.XMLHTTP"); //validamos el navegador en el que estamos
